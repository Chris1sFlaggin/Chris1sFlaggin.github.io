---
layout: cyber-post
title: "StackJet - Stack Buffer Overflow Writeup"
date: 2025-01-26
categories: 
  - writeups
  - stack
tags:
  ...
  - pwn
  - ctf
excerpt: "Writeup della challenge StackJet - exploiting stack buffer overflow"
---
...
L’aspetto centrale era capire il comportamento del JIT e come manipolarlo, ecco il codice disassemblato da un famoso disassembler:
```c
...
* **`OP_DUP` (`\x02`)**: Duplica il valore in cima allo stack. (Es. se lo stack è `[A]`, diventa `[A, A]`).
* **`OP_SWAP` (`\x03`)**: Scambia di posto i due valori in cima allo stack. (Es. se lo stack è `[A, B]`, diventa `[B, A]`).
...
Se provo a pushare un valore e ad eseguire ```ADD``` non riesco a farlo perchè mi dice che non ho abbastanza valori sullo stack (ne abbiamo messo solo 1).

Inizio quindi a guardare la logica delle operazioni e vedo tra tutte una risulta particolarmente curiosa: la funzione ```SWAP```, poichè ci permette di operare senza andare a chiamare ```track_stack()```.
...
Ora dobbiamo solo pensare a cosa pushare, io ho optato per uno shellcode che ogni 8 byte ne salta 3 poichè ci sono i byte delle operazioni tradotte dal compilatore del programma da evitare di eseguire, per farlo ho scelto un'implementazione che mi stampasse a schermo ogni 8 byte per semplificarmi la vita(nella push si devono mandare 8 byte):
```python
from pwn import *
...
Ora basta poi rimuovere dallo stack lo shellocode allocato in modo che venga salvato solo in RWX ed il gioco è fatto, ecco l'exploit completo:
```python
from pwn import *
...
> 💭 Se hai bisogno di chiarimenti scrivimi pure su discord: *m9b3\_0b20ck*.
